...
{
    [TestClass]
    public class NextFitAllocatorTest
    {
        [TestMethod]
        public void TestAlloction()
        {
            var manager = MemoryManagerFactory.CreateMemoryManager(0u, 1024, MemoryManagerFactory.AllocationStrategy.NextFit);
        ...
        {
            var manager = MemoryManagerFactory.CreateMemoryManager(65536u, 1024, MemoryManagerFactory.AllocationStrategy.NextFit);
            {
        ...
        {
            var manager = MemoryManagerFactory.CreateMemoryManager(128u, 1024, MemoryManagerFactory.AllocationStrategy.NextFit);
            {
        ...
        public void TestNextFit()
        {
...

            {
                ...
                var memory3Val = manager.AllocateMemory(64) ?? 0;
                // |32| 64 |32| 128 96 16 64 |80| 16 |16| 64 |64| 16 |16| 64 |128| 128
                var memory4Val = manager.AllocateMemory(48) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 |32| 16 |16| 64 |64| 16 |16| 64 |128| 128
                var memory5Val = manager.AllocateMemory(0) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 |32| 16 |16| 64 |64| 16 |16| 64 |128| 128
                var memory6Val = manager.AllocateMemory(32) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 32 16 |16| 64 |64| 16 |16| 64 |128| 128
                var memory7Val = manager.AllocateMemory(32) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 32 16 |16| 64 32 |32| 16 |16| 64 |128| 128
                var memory8Val = manager.AllocateMemory(8) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 32 16 |16| 64 32 8 |24| 16 |16| 64 |128| 128
                var memory9Val = manager.AllocateMemory(128) ?? 0;
                // |32| 64 |32| 128 96 16 64 48 32 16 |16| 64 32 8 |24| 16 |16| 64 128 128
                var memory10Val = manager.AllocateMemory(16) ?? 0;
                // 16 |16| 64 |32| 128 96 16 64 48 32 16 |16| 64 32 8 |24| 16 |16| 64 128 128
                {
                    var freeList = manager.GetFreeMemories();
                    var freeArr = new int[] { 16, 32, 16, 24, 16 };
                ...
                Assert.IsTrue(manager.FreeMemory(memories[5]));
                Assert.IsTrue(manager.FreeMemory(memories[7]));
                // 16 |16| 64 |32| 128 96 16 64 48 32 |96| 32 8 |24| 16 |16| 64 128 128
                {
                    var freeList = manager.GetFreeMemories();
                    var freeArr = new int[] { 16, 32, 96, 24, 16 };
                    var q = new Queue<int>(freeArr);
                    Assert.IsTrue(freeList.Count == freeArr.Length);
                    foreach (var memoryBlockInfo in freeList)
                    {
                ...
                Assert.IsTrue(manager.FreeMemory(memory10Val));
                Assert.IsTrue(manager.FreeMemory(memories[1]));
                // |96| |32| 128 96 16 64 48 32 |96| 32 8 |24| 16 |16| 64 128 128
                var memory11Val = manager.AllocateMemory(96) ?? 0;
                // 96 |32| 128 96 16 64 48 32 |96| 32 8 |24| 16 |16| 64 128 128
                Assert.IsTrue(memory11Val == 128u);
                {
                    var freeList = manager.GetFreeMemories();
                    var freeArr = new int[] { 32, 96, 24, 16 };
                    var q = new Queue<int>(freeArr);
                    Assert.IsTrue(freeList.Count == freeArr.Length);
                    foreach (var memoryBlockInfo in freeList)
                    {
